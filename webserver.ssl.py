...
def main():
    ...
    proxy_port = 444  # Change this to the desired proxy port
    with socketserver.TCPServer(('', proxy_port), ProxyHandler) as server:
        print(f"Proxy server is running on port {proxy_port}")
        server.serve_forever()

if __name__ == "__main__":
    external_port = 443  # External port you want to forward
    internal_port = 443   # Internal port of the downstream nginx server
    main()
